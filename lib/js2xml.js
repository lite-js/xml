...
function js2xml(obj) {
  if (obj.type === 'Comment') return tplComment(obj);
  if (obj.type === 'TextNode') return obj.text || '';
  if (obj.type === 'ProcessingInstruction') return tplProcessingInstruction(obj);
  const content = lang.map(obj.children || [], (child) => js2xml(child)).join('');
  if (obj.type === 'Document') return content;
...
module.exports = (obj, options) => {
  ...
  if (options.beautify) return xmlFormat.beautify(result, options.step);
